		...
		if ( ! isset( $_POST['woocommerce_checkout_update_totals'] ) && wc_notice_count( 'error' ) == 0 ) {
			...
			try {
				...
				if ( WC()->cart->needs_payment() ) {
					...
					//echo "SELECT * FROM wp_postmeta WHERE meta_value = '.$sku'";
...
'IPNNotificationURL' => $base_url . '/callbacks/',
'Memo' => '',
'Pin' => '',
'PreapprovalKey' => '',
'ReturnURL' => $base_url . '/paypal-confirm/',
...
return;//die( 'Error: '.$PayPalResult['Errors'][0]['Message'] );
}
