...
$sku = $url_parts['item_number_1'];

			global $wpdb;
			$res = $wpdb->get_results( 'SELECT * FROM wp_postmeta WHERE meta_value = '.$sku);
			$post_id =$res[0]->post_id;

			$res2 = $wpdb->get_results( 'SELECT term_taxonomy_id FROM wp_term_relationships  WHERE object_id = '.$post_id);
			//print_r($res2);
			
			$all_termid = array();

			foreach($res2 as $rss){
				$all_termid[] = $rss->term_taxonomy_id;
			}
			$all_termid=implode(',', $all_termid);
			$res3 = $wpdb->get_results( 'SELECT term_id FROM wp_term_taxonomy  WHERE  term_taxonomy_id in('.$all_termid.') AND taxonomy = "shop_vendor"');
			//print_r($res3[0]->term_id);

			$res4 = $wpdb->get_results( 'SELECT * FROM wp_options  WHERE  option_name = "shop_vendor_'.$res3[0]->term_id.'"');
			//print_r($res4);
			$option_val = array();
			$option_val = unserialize($res4[0]->option_value);
			//print_r($option_val);
			$paypalEmail = $option_val['paypal_email'];
			$vendorShare = $option_val['commission'];

			 $siteShare = 100-$vendorShare;

//********************************************//vendor commissions
$receivers[] = array(
'Amount' => $vendorShare+$siteShare, // Required. Amount to be paid to the receiver.
'Email' => $paypalEmail, // Receiver's email address. 127 char max.
'InvoiceID' => 123, // The invoice number for the payment. 127 char max.
...
'ApplicationID' => 'APP-1RG01394B8393230T',
'DeviceID' => '',
'IPAddress' => $_SERVER['REMOTE_ADDR'],
'APIUsername' => 'guedesign_api1.gmail.com',
'APIPassword' => 'GARH7L8XAJTZU2JA',
'APISignature' => 'AgWWO0jCmXuJndwqcjjlqm4EDQgfAWZFJqdyy5fvkqBlITxpYyiO6UdS',
'APISubject' => ''
);
...
'ActionType' => 'PAY',
'CancelURL' => 'http://admin.keenlo.com',
'CurrencyCode' => 'AUD',
'FeesPayer' =>'SECONDARYONLY',
'IPNNotificationURL' => 'http://admin.keenlo.com',
'Memo' => '',
'Pin' => '',
'PreapprovalKey' => '',
'ReturnURL' => 'http://admin.keenlo.com',
...
echo 'Error: '.$PayPalResult['Errors'][0]['Message'];
return;
}*/
