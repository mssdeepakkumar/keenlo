...
class WC_Social_Login_Provider_Profile {
	...
	public function __construct( array $profile ) {
		 ...
		 */
		$this->profile = apply_filters( 'wc_social_login_' . $this->get_provider_id() . '_profile', $profile );
	}
	...
	public function __call( $method, $args ) {
		...
		if ( 0 === strpos( $method, 'get_' ) ) {
			...
			return $this->get_profile_info_value( $method );
		}
		...
		if ( 0 === strpos( $method, 'has_' ) ) {
			...
			return isset( $this->profile['info'][ $method ] );
		}
	 ...
	 * @return array raw profile
	 */
	private function get_profile_info_value( $key ) {

		return isset( $this->profile['info'][ $key ] ) ? $this->profile['info'][ $key ] : '';
	}
	...
	public function update_customer_profile( $user_id, $new_customer ) {
		...
		update_user_meta( $user_id, '_wc_social_login_' . $this->get_provider_id() . '_profile', $this->get_raw_profile() );
